...
import com.ke.mianshiya.model.enums.UserDeviceEnum;
...
public class DeviceUtils {
    ...
    public static String getRequestDevice(HttpServletRequest request) {
        ...
        // 默认值是 PC
        String device = UserDeviceEnum.PC.getValue();
        //是否为小程序
        if (isMiniProgram(userAgentStr)) {
            device = UserDeviceEnum.MINI_PROGRAM.getValue();
        }else if (isPad(userAgentStr)){
            //是否为ipad
            device = UserDeviceEnum.Pad.getValue();
        }else if (userAgent.isMobile()){//直接使用hutool解析出来user-agent对象判断是否是移动端
            device = UserDeviceEnum.MOBILE.getValue();
        }
